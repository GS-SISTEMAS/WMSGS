...
namespace TESO.LetrasEnvioEmailNeo
{
    class Program
    {
        ...
        public static void EnviarCorreos(Int32 idEmpresa, string ruc_empresa, string esquemaREC)
        {
            ...
            if (!Directory.Exists(@"C:\Email\NeoAgrum\"))
                Directory.CreateDirectory(@"C:\Email\NeoAgrum\");

            string[] Archivos = Directory.GetFiles(@"C:\Email\NeoAgrum\");
            foreach (string pdfs in Archivos)
                File.Delete(pdfs);

            foreach (DataRow row in clientes.Rows)
            {
                Archivos = Directory.GetFiles(@"C:\Email\NeoAgrum\");
                foreach (string pdfs in Archivos)
                    File.Delete(pdfs);
                ...
                foreach (string opdoc_venta in lstopDoc_Venta)
                {
                    ...
                    else
                    {
                        ...
                        File.WriteAllBytes(@"C:\Email\NeoAgrum\" + nombreletra, bytes);
                    }
