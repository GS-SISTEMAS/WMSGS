...
namespace TESO.LetrasEnvioEmail
{
    class Program
    {
        static void Main(string[] args)
        {
            string ruc_empresa = string.Empty;
            ...
            rutapdf = ConfigurationSettings.AppSettings["RUTAPDF"].ToString();
            idEmpresa = Convert.ToInt32(ConfigurationSettings.AppSettings["IDEMPRESA"]);

            EnviarCorreos(idEmpresa, ruc_empresa, esquemaREC,rutapdf);
        ...
        public static void EnviarCorreos(Int32 idEmpresa,string ruc_empresa,string esquemaREC,string rutapdf)
        {
            bool ErrorWebMethod = false;
            ...
            string[] Archivos = Directory.GetFiles(rutapdf);
            foreach (string pdfs in Archivos)
                File.Delete(pdfs);

            foreach (DataRow row in clientes.Rows)
            {
                Archivos = Directory.GetFiles(rutapdf);
                foreach (string pdfs in Archivos)
                    File.Delete(pdfs);
                ...
                foreach (string opdoc_venta in lstopDoc_Venta)
                {
                    ...
                    else
                    {
                        ...
                        File.WriteAllBytes(rutapdf + nombreletra, bytes);
                    }
                ...
                try
                {
                    if (!ErrorWebMethod)
                    {
                        EnviarCorreo(EmailCliente, EmailVendedores, rutapdf);
        ...
        private static void EnviarCorreo(List<string> lstclientes,List<string> lstvendedores,string rutapdf)
        {
            try
            {
